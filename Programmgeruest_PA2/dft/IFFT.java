...
public class IFFT {
    ...
    public static Complex[] ifft(Complex[] c) {
        ...
        // double omega = Math.exp(2 * Math.PI / n); // TODO: add i, simplify using periodität
        // TODO: make omega complex


        Complex[] v = new Complex[n];
        for (int j=0; j<m; j++) {
            v[j]   = z1[j];// + Math.pow(omega, j) * z2[j];
            v[m+j] = z1[j];// - Math.pow(omega, j) * z2[j];
        }
